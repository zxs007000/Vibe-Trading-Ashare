...
## 6.1 已提交结果（250 只子样本 + 全市场 5500 只, WFA 4 折）

> 仓库同时提交两套结果: (1) **250 只随机子样本**(`WFA_MAX_STOCKS=250`, seed=42)的跑通基线,
> 方法学与全量一致, 详见 [`results/proto_v4_chunked_250_results.md`](./results/proto_v4_chunked_250_results.md)
> 与 [`results/shap_report.md`](./results/shap_report.md); (2) **全市场 5500 只** 由分块流式版
> `xgb_wfa_proto_v4_chunked.py` 在 8GB cgroup 内完整跑通, 详见
> [`results/proto_v4_chunked_full_results.md`](./results/proto_v4_chunked_full_results.md)。

### 6.1.1 全市场结果（5500 只, WFA 4 折, 分块流式版）

> 已在 8GB cgroup 内完整跑通: 峰值 `cg.current`≈80%(≈6.5GB), 关键为 **float32 分片 + 每分片 `close`+`sleep`+`release_unused` 回收页缓存 + 增量训练/断点续跑**。
> 训练行数(IS)封顶 `WFA_TRAIN_CAP=500,000` 子抽样以控内存; 全市场横截面特征已覆盖所有股票, 不影响覆盖度。

**各折 OOS 表现**(标签=未来收益排前 30% 的概率; 融合=5/20/60 日三周期概率均值):

| 折 | IS 区间 | OOS 区间 | 单周期5日 AUC | 融合 AUC(5/20/60日) | 融合 IC(5/20/60日) | IS 训练行数 |
|---|---|---|---|---|---|---|
| 1 | 2018-10~2021-10 | 2021-10~2022-10 | 0.535 | 0.538 / 0.541 / 0.522 | -0.002 / -0.008 / -0.035 | 346,451 |
| 2 | 2019-10~2022-10 | 2022-10~2023-10 | 0.534 | 0.538 / 0.538 / 0.530 | +0.011 / +0.002 / -0.021 | 500,000 |
| 3 | 2020-10~2023-10 | 2023-10~2024-10 | 0.529 | 0.525 / 0.552 / 0.550 | +0.049 / +0.076 / +0.038 | 500,000 |
| 4 | 2021-10~2024-10 | 2024-10~2025-10 | 0.536 | 0.522 / 0.553 / 0.570 | +0.054 / +0.087 / +0.102 | 500,000 |
| **均值** | — | — | **0.5335** | **0.5307 / 0.5459 / 0.5429** | **+0.028 / +0.039 / +0.021** | — |

**解读**:
- 全市场 AUC 稳定 ~0.53, 与 250 只子样本结论一致(>0.5 即微弱但一致的样本外区分度)。
- 融合 IC 在折3/折4 明显为正(尤其 60 日 +0.10), 折1/折2 偏负 —— **样本外信号在近期(2023-2025)强于早期(2019-2022)**。
- **因子重要性 Top**: `pca_absorp_pen`(PCA 吸收比率惩罚项)、`crowd_liq_pct`/`crowd_mom_pct`(交易行为拥挤度)、`netprofit_yoy`(盈利增速)、`mkt_roe_mean`(市场 ROE 环境) 等。
- **拥挤度特征总 Gain 占比 12.9%**, 筹码结构 1.8% —— 拥挤度因子在全市场面板中确有稳定贡献, 筹码结构贡献偏小(数据湖缺 tick/换手代理所致)。

### 6.1.2 250 只代表性子样本结果（WFA 4 折, 方法学一致）

> 受 8GB 内存上限, 早期提交的基线为 250 只随机子样本(`WFA_MAX_STOCKS=250`, seed=42);
> 分块流式版 `xgb_wfa_proto_v4_chunked.py` 现已可在同等内存跑全市场 5500 只(见 §6.1.1)。

**250 只子样本各折 OOS 表现**(标签=未来收益排前 30% 的概率; 融合=5/20/60 日三周期概率均值):

| 折 | 单周期5日 AUC | 融合 AUC(5/20/60日) | 融合 IC(5/20/60日) |
|---|---|---|---|
| 1 | 0.528 | 0.527 / 0.536 / 0.529 | +0.013 / +0.029 / +0.007 |
| 2 | 0.536 | 0.535 / 0.544 / 0.530 | +0.009 / +0.025 / +0.007 |
| 3 | 0.528 | 0.517 / 0.546 / 0.539 | +0.036 / +0.082 / +0.069 |
| 4 | 0.538 | 0.528 / 0.559 / 0.587 | +0.056 / +0.090 / +0.115 |
| **均值** | **0.5325** | **0.5266 / 0.5464 / 0.5464** | **+0.0285 / +0.0564 / +0.0497** |

**解读**:
- 融合 AUC 稳定在 **0.53~0.55**(>0.5 即有微弱但一致的样本外区分度); 单周期5日 AUC≈0.53。
- 融合 IC 全正(0.03~0.12), 且随 horizon 拉长(20/60日)IC 更高 —— 信号更偏中期。
- 因子重要性 Top: 多为「市场环境 × 个股」交互特征(`ix_mkt_*__*`)、`pca_absorp_delta_x_macro`(PCA 吸收比率/拥挤度代理)、`netprofit_yoy`(盈利增速)、`mkt_roe_mean`(市场 ROE 环境)。
- 拥挤度 / 筹码结构 两新维度 Gain 占比分别为 **2.5% / 2.8%** —— 占比不高但非零, 证明它们携带了增量信息(详见 SHAP 报告跨折方向一致性)。
