...
本分块版用「分组切片 + 流式落盘」把峰值内存压到单折训练矩阵量级(~4-7GB)，全市场 5500 只在 8GB 内可跑：
  ...
  Phase B  按"年"切片，每片载入"该窗内全部股票"算截面特征
           (市场环境/拥挤度/PCA/交互/z-score)，写盘释放 → 峰值 ≈ 5448×252×365 ≈ 2.5GB(float32)
  Phase C  每折只读取覆盖该折日期窗的特征分片，float32 增量拼矩阵后训练
           → 峰值 ≈ 单折训练矩阵(~3.5GB) + DMatrix 拷贝(~3.5GB) ≈ 7GB

截面运算(市场环境/拥挤度/PCA/z-score/交互)只需要"某一天的所有股票同时在线"，
不需要"所有股票的所有历史同时在线"——这正是分块可行的理论依据。

容错: `WFA_RESUME=1` 断点续跑(复用已落盘的 _base / _feat 分片，避免环境杀进程后重头来)。
注: 切片内 winsorize 为该分片内的分位(非全历史全局分位)，属可接受的局部近似(原型级)。
"""
import os, time, shutil, json, warnings
import numpy as np
import pandas as pd
...
RESUME = os.environ.get("WFA_RESUME", "0") == "1"          # 断点续跑(复用已落盘分片)
TRAIN_YEARS = 3
TEST_YEARS = 1
STEP_YEARS = 1
RANDOM_STATE = 42
...
def _global_feats_from(path):
    ...
    return [c for c in cols if c not in NON_FEAT]
...
def main():
    ...
    print(f"[1] 股票数: {len(codes)} | RESUME={RESUME}", flush=True)
    ...
    print("[2] Phase A 逐只股票基础特征(价量/基本面/筹码) → 落盘...", flush=True)
    meta = {}
    ...
    if RESUME and os.path.exists(meta_path):
        ...
        print(f"    RESUME: 从 _meta.json 载入 {len(meta)} 只基础特征元信息(跳过重建)", flush=True)
    else:
        for c in codes:
    ...
    # ── Phase B: 按"年"切片, 每片载入该窗内全部股票算截面特征 → 落盘(峰值~2.5GB float32) ──
    print("[3] Phase B 日期切片截面特征(市场环境/拥挤度/PCA/交互/z-score)...", flush=True)
    chunks = []
    start = all_min
    ci = 0
    base_crowds = []
    GLOBAL_FEATS = None
    ENV_N, CROWD_N = 8, 18   # 由算法固定(7 个 mkt_* + is_extreme_macro=8; 3 基础×6 展开=18)
    n_rows_est = 0
    while start <= all_max:
        ...
        for c, (m0, m1) in meta.items():
            if m1 >= warm0 and m0 <= cd1:
                ...
                del df
        if parts:
            # 关键: concat 后立即转 float32, 否则后续 interactions 在 float64 下把面板撑到 ~6-8GB(全市场后期分片必 OOM)
            ...
            sub.to_parquet(_fpath)   # 已是 float32
    ...
    def load_window(d0, d1):
        """读取覆盖 [d0,d1) 的特征分片, 预分配拼成 float32 矩阵, 避免 concat 临时翻倍顶爆 8GB。"""
        Xp, swp, codep, datep = [], [], [], []
        ...
        if not Xp:
            return None
        # 预分配, 避免 np.concatenate 临时再建一份导致峰值翻倍
        n = sum(x.shape[0] for x in Xp)
        nf = Xp[0].shape[1]
        ...
        for k in range(len(Xp)):
            ...
            X[off:off + m] = Xp[k]
            sw[off:off + m] = swp[k]
            code[off:off + m] = codep[k]
            date[off:off + m] = datep[k]
            for hz in HORIZONS:
                cls[hz][off:off + m] = clsh[hz][k]
                fwd[hz][off:off + m] = fwdh[hz][k]
            off += m
        del Xp, swp, codep, datep, clsh, fwdh
    ...
    for i, (is_s, is_e, oos_s, oos_e) in enumerate(folds, 1):
        ...
        if osd is None or len(osd["X"]) < 100:
            print(f"    折{i}: OOS 样本不足跳过", flush=True); continue
        Xte = osd["X"]
    ...
    lines = ["# XGBoost 因子挖掘 · WFA 原型 v4（分块流式版，全市场）", "",
             ...
             f"- 内存策略: PhaseA逐只落盘 → PhaseB按年切片截面特征(float32) → PhaseC按需读分片+float32增量拼矩阵",
             f"  全市场 5500 只可在 8GB cgroup 内运行(峰值≈单折训练矩阵 ~3.5GB + DMatrix ~3.5GB ≈ 7GB)",
    ...
    lines += ["", "## 结论", "",
              ...
              "通过 PhaseA逐只落盘 + PhaseB按年切片截面特征(float32) + PhaseC按需读分片+float32增量拼矩阵，",
