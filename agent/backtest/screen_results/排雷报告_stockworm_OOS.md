...
3. **待办（关键）**：walk-forward 逐窗口方向自适应（替换静态定向），预期能修复跨行情负号。
4. 可选：拉**日线**长历史（2023~2026）覆盖 2024-02 急跌，供日频因子做更长 OOS。
5. 在可联网环境用 Section 四的 PAT 命令完成 GitHub 推送（含 pull 脚本 + 路径改动 + 本报告）。
